...
main(){
    ...
    MSI_PRINCIPAL_ID=$(az identity list -g $RESOURCE_GROUP --query "[?name=='$MSI_NAME'].{principalId:principalId}" -o tsv)
    if [ -z "$MSI_PRINCIPAL_ID" ]; then
        _information "Managed Identity ${MSI_NAME} was not Found. Creating new MSI.."
        MSI_PRINCIPAL_ID=$(az identity create -n $MSI_NAME -g $RESOURCE_GROUP | jq -r ".principalId")
        _information "MSI ${MSI_NAME} created! Principal Id: $MSI_PRINCIPAL_ID"
    else
        _information "Managed Identity ${MSI_NAME} exists. Principal Id: $MSI_PRINCIPAL_ID"
    fi
